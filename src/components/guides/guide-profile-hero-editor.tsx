        ...
        <div className="relative flex flex-col justify-start p-6 sm:p-8 lg:p-8">
          {!editing ? (
            ...
            <form id={formId} onSubmit={saveProfile} className="space-y-4">
              <div><input name="name" defaultValue={guide.name} aria-label="Guide name" required className={`${inputClassName} font-heading text-3xl font-semibold tracking-tight sm:text-4xl`} /><input name="location" defaultValue={guide.location} aria-label="Location" required className={`${inputClassName} mt-2 text-sm font-semibold uppercase tracking-[0.25em] text-muted-foreground`} /></div>
              <p className="flex items-center gap-2 text-base leading-7 text-muted-foreground"><input name="experienceYears" type="number" min="0" defaultValue={guide.experienceYears} aria-label="Years of experience" className="w-16 rounded-lg border border-border/80 bg-background px-2 py-1 font-heading text-lg font-semibold text-emerald-700 outline-none focus:ring-2 focus:ring-ring/20 dark:text-emerald-400" /> years experience</p>
              <textarea name="bio" defaultValue={guide.bio} aria-label="Bio" required rows={4} className={`${inputClassName} resize-none text-sm leading-6 text-muted-foreground`} />
              <div><p className="mb-2 text-[0.7rem] font-semibold uppercase tracking-[0.25em] text-muted-foreground">Certifications</p><textarea name="certifications" defaultValue={guide.certifications.map((certification) => certification.title).join("\n")} rows={3} className={`${inputClassName} resize-none text-sm`} /></div>
              <GuideSportsField sports={guide.sports} />
              <div><p className="mb-2 text-[0.7rem] font-semibold uppercase tracking-[0.25em] text-muted-foreground">Languages</p><textarea name="languages" defaultValue={guide.languages.join("\n")} rows={2} className={`${inputClassName} resize-none text-sm`} /></div>
