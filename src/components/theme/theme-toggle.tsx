...
export function ThemeToggle({ compact = false, responsive = false }: { compact?: boolean; responsive?: boolean }) {
  ...
  return (
    <button
      ...
      className={cn(
        ...
        compact ? "h-7 w-11" : responsive ? "h-7 w-11 md:h-9 md:w-[4.25rem]" : "h-9 w-[4.25rem]",
      )}
    >
      ...
      <span
        className={cn(
          ...
          compact ? "h-4 w-4" : responsive ? "h-4 w-4 md:h-7 md:w-7" : "h-7 w-7",
          mounted && isDark ? "left-1" : "right-1",
        )}
      >
        {mounted ? (
          isDark ? (
            <Sun className={compact ? "h-2.5 w-2.5" : responsive ? "h-2.5 w-2.5 md:h-4 md:w-4" : "h-4 w-4"} />
          ) : (
            <Moon className={compact ? "h-2.5 w-2.5" : responsive ? "h-2.5 w-2.5 md:h-4 md:w-4" : "h-4 w-4"} />
          )
        ) : (
          <span className={compact ? "h-2.5 w-2.5" : responsive ? "h-2.5 w-2.5 md:h-4 md:w-4" : "h-4 w-4"} />
        )}
      </span>
