...
export function BookingCard({ booking }: { booking: BookingCardData }) {
  ...
  const canReview = booking.showReview && booking.status === "COMPLETED";
  const handleReviewSubmitted = useCallback(() => setReviewOpen(false), []);
    ...
    <div className="overflow-hidden rounded-[1rem] border border-border/70 bg-background/60 transition-colors hover:border-border">
      <div className="group flex flex-col sm:flex-row">
        <button
          ...
          className="relative h-32 w-full shrink-0 overflow-hidden bg-muted/60 sm:h-auto sm:w-36"
        >
          <Image
            src={booking.image}
            alt={booking.title}
        ...
        <div className="flex min-w-0 flex-1 flex-col">
            ...
            <div className="flex items-center justify-between gap-3 border-b border-border/60 px-3 py-2">
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  className="h-6 rounded-full px-2.5 text-[0.65rem] tracking-wide"
                >
                  {reviewOpen
                    ? "Close review"
                    : booking.review
          ...
          <button
            ...
            aria-expanded={open}
            onClick={() => setOpen((value) => !value)}
            className="flex flex-1 flex-col justify-between gap-2 p-3 text-left"
          >
            <div>
